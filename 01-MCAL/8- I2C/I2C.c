...
FlagSTATUS I2C_GetFlagSTATUS(I2C_REG_t *I2Cx, Status Flag)
{
...
void I2C_AcknowledgeConfig(I2C_REG_t *I2Cx, Fuctional_State NewState)
{
	if(NewState == ENABLE)
	{
...
void I2C_GenerateSTART(I2C_REG_t *I2Cx, Fuctional_State NewState, Repeated_Start Start)
{
...
void I2C_GenerateSTOP(I2C_REG_t *I2Cx, Fuctional_State NewState)
{
	if(NewState == ENABLE)
	{
		/*Generate a START Condition*/
...
void I2C_SendAddress(I2C_REG_t *I2Cx, uint16_t Address, I2C_Direction Direction)
{
	Address = (Address << 1);
...
void Slave_States(I2C_REG_t *I2Cx,Slave_State State)
{
...
void I2C_IRQ_EN(NVIC_REG_t* NVICx, I2C_REG_t* I2Cx)
{
	if( I2Cx == I2C1)
	{
		NVIC_IRQ_EN(NVICx->NVIC_ISER[0], NVIC_I2C1_EV);
		NVIC_IRQ_EN(NVICx->NVIC_ISER[1], (NVIC_I2C1_ER - 32));
	}
	else if( I2Cx == I2C2)
	{
		NVIC_IRQ_EN(NVICx->NVIC_ISER[1], (NVIC_I2C2_EV - 32));
		NVIC_IRQ_EN(NVICx->NVIC_ISER[1], (NVIC_I2C2_ER - 32));
	}
}


void I2C_IRQ_DIS(NVIC_REG_t* NVICx, I2C_REG_t* I2Cx)
{
	if( I2Cx == I2C1)
	{
		NVIC_IRQ_EN(NVICx->NVIC_ICER[0], NVIC_I2C1_EV);
		NVIC_IRQ_EN(NVICx->NVIC_ICER[1], (NVIC_I2C1_ER - 32));
	}
	else if( I2Cx == I2C2)
	{
		NVIC_IRQ_EN(NVICx->NVIC_ICER[1], (NVIC_I2C2_EV - 32));
		NVIC_IRQ_EN(NVICx->NVIC_ICER[1], (NVIC_I2C2_ER - 32));
	}
}
...
void MCAL_I2C_Init(I2C_REG_t *I2Cx , I2C_Config_t *I2C_Config)
{
	...
	if(I2C_Config->I2C_Mode == I2C_Mode_I2C)
	{
		...
		pclk1 = MCAL_RCC_GetPCLK1Freq(HSI);
	...
	if( I2C_Config->P_Slave_Event_CallBack != NULL) // that enable IRQ States Mode
	{
		...
		if(I2Cx == I2C1)
		{
			I2C_IRQ_EN(NVIC, I2C1);
		}
		else if(I2Cx == I2C2)
		{
			I2C_IRQ_EN(NVIC, I2C2);
		}
		I2Cx->I2C_SR1 = 0;
		I2Cx->I2C_SR2 = 0;
	}
 ...
 * Note			- none
 */
void MCAL_I2C_DeInit(I2C_REG_t *I2Cx)
{
	if( I2Cx == I2C1 ){
		I2C_IRQ_DIS(NVIC, I2C1);
		MCAL_RCC_PERIPHERAL_DIS(APB1, RCC_I2C1);
	}
	else{
		I2C_IRQ_DIS(NVIC, I2C2);
		MCAL_RCC_PERIPHERAL_DIS(APB1, RCC_I2C2);
	}
}
 ...
 */
void MCAL_I2C_Set_Pins(I2C_REG_t *I2Cx)
{
	if(I2Cx == I2C1){
		MCAL_RCC_PERIPHERAL_EN(APB1, RCC_I2C1);

		/* PB6  SCL
		PB7  SDA */

		//PB6
		MCAL_GPIO_INIT(GPIOB, PIN6, OUTPUT_AF_OD, SPEED_10MHz);

		//PB7
		MCAL_GPIO_INIT(GPIOB, PIN7, OUTPUT_AF_OD, SPEED_10MHz);
	}
	else{
		MCAL_RCC_PERIPHERAL_EN(APB1, RCC_I2C1);
		...
		//PB10
		MCAL_GPIO_INIT(GPIOB, PIN10, OUTPUT_AF_OD, SPEED_10MHz);

		//PB11
		MCAL_GPIO_INIT(GPIOB, PIN11, OUTPUT_AF_OD, SPEED_10MHz);
	}
}
 ...
 * Note 			- None
 */
void MCAL_I2C_Master_TX(I2C_REG_t *I2Cx, uint16_t DevAddr ,uint8_t* dataOut , uint32_t datalen , Stop_Condition Stop , Repeated_Start Start)
{
	int i = 0;
 ...
 * Note 			- None
 */
void MCAL_I2C_Master_RX(I2C_REG_t *I2Cx, uint16_t DevAddr ,uint8_t* dataOut , uint32_t datalen , Stop_Condition Stop , Repeated_Start Start)
{
	int i = 0;
 ...
 */
void MCAL_I2C_SlaveSendData(I2C_REG_t *I2Cx , uint8_t data)
{
	I2Cx->I2C_DR = data;
}
 ...
 *
 */
uint8_t MCAL_I2C_SlaveReceiveData(I2C_REG_t *I2Cx)
{
